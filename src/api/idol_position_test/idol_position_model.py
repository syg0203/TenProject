...
class idol_position:
    def __init__(self):
        ...
        self.label_map = {'label_1' : {'1' : '남돌', '2' : '여돌'},'label_2' : {'1' : '메인', '2' : '서브'},'label_3' : {'1' : '보컬', '2' : '댄서', '3':'래퍼'},'label_4_M' : {'1' : '비쥬얼', '2' : '피지컬' , '3' : '예능'},'label_4_W' : {'1' : '큐티', '2' : '섹시', '3' : '청순'}}
        self.idx2label = []
        ...
        self.model.load_state_dict(torch.load(f'{base_path}model_weights(ResNet_IDOL).pt', map_location=torch.device('cpu')))
...
async def upload_result(file: UploadFile):
    ...
    file_path = os.path.join(UPLOAD_DIR, str(uuid.uuid4()) + '.' + file.filename.split('.')[-1])
